@extends('frontend.layouts.app')
    <style>
        @media only screen and (max-width: 700px) {
            .dash_tabs_movable_right {
                padding: 24px 0px;
                padding-bottom: 0px;
            }
            .order_top_div {
                background: url({{ asset('frontend/BrandSparkz/assets/img/contact_box_bg.png') }});
                padding-bottom: 24px;
                border-radius: 20px;
            }
        }
    </style>

@section('content')
    



@php
    //dump($order);
    $status = $order->orderDetails->first()->delivery_status;
    $refund_request_addon = \App\Models\Addon::where('unique_identifier', 'refund_request')->first();
@endphp



    <section class="dashboard_page">
        <div class="dash_s1">
            <div class="container">
                <div class="dash_s1_content">
                    <h1 class="dash_title" id="dash_title">
                        My account
                    </h1>
                    <p class="dash_subtitle">
                        From your account Brand Sparkzboard you can vier your recent orders, manage your billing
                        addressed and edit your password and account details.
                    </p>
                </div>
            </div>
            <img src="{{ asset('frontend/BrandSparkz/assets/img/dash_cutout.png') }}" alt="" class="img-fluid dash_cutout">
        </div>

        <div class="dash_s2">

            <div class="dash_tabpanel">


                <div class="dash_tabs_left">
                    <div class="dash_tabs_left_inner">
                        <button class="btn dash_tab_btn">
                            <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"
                                fill="none">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M36.4701 11.5101C35.3301 6.82998 28.6699 6.82998 27.5299 11.5101C27.3597 12.2133 27.0258 12.8665 26.5555 13.4163C26.0852 13.9662 25.4917 14.3972 24.8233 14.6744C24.1549 14.9516 23.4306 15.067 22.7091 15.0114C21.9877 14.9557 21.2896 14.7305 20.6716 14.3541C16.5555 11.8461 11.8454 16.5561 14.3534 20.6722C15.9735 23.3302 14.5365 26.7983 11.5124 27.5333C6.82921 28.6703 6.82921 35.3334 11.5124 36.4674C12.2158 36.6379 12.8691 36.972 13.419 37.4427C13.9688 37.9133 14.3998 38.5072 14.6767 39.176C14.9536 39.8447 15.0686 40.5694 15.0125 41.291C14.9563 42.0126 14.7305 42.7107 14.3534 43.3286C11.8454 47.4446 16.5555 52.1547 20.6716 49.6467C21.2895 49.2696 21.9876 49.0438 22.7093 48.9876C23.4309 48.9315 24.1556 49.0465 24.8243 49.3234C25.4931 49.6003 26.0869 50.0313 26.5576 50.5811C27.0283 51.131 27.3624 51.7843 27.5329 52.4877C28.6699 57.1708 35.3331 57.1708 36.4671 52.4877C36.6381 51.7846 36.9726 51.1318 37.4434 50.5823C37.9142 50.0328 38.508 49.6022 39.1765 49.3254C39.845 49.0485 40.5694 48.9334 41.2909 48.9892C42.0123 49.045 42.7104 49.2703 43.3284 49.6467C47.4445 52.1547 52.1546 47.4446 49.6466 43.3286C49.2702 42.7106 49.0449 42.0125 48.9891 41.2911C48.9332 40.5697 49.0484 39.8453 49.3253 39.1767C49.6021 38.5082 50.0327 37.9144 50.5822 37.4437C51.1317 36.9729 51.7846 36.6384 52.4876 36.4674C57.1708 35.3304 57.1708 28.6673 52.4876 27.5333C51.7842 27.3629 51.1309 27.0287 50.5811 26.5581C50.0312 26.0874 49.6002 25.4935 49.3233 24.8248C49.0464 24.1561 48.9314 23.4314 48.9875 22.7098C49.0437 21.9882 49.2695 21.29 49.6466 20.6722C52.1546 16.5561 47.4445 11.8461 43.3284 14.3541C42.7105 14.7311 42.0124 14.957 41.2907 15.0131C40.5691 15.0693 39.8444 14.9542 39.1757 14.6773C38.5069 14.4004 37.9131 13.9695 37.4424 13.4196C36.9717 12.8698 36.6376 12.2165 36.4671 11.5131L36.4701 11.5101Z"
                                    stroke="#3C3C3C" stroke-width="2" />
                                <path
                                    d="M37.3333 32C37.3333 34.9455 34.9455 37.3333 32 37.3333C29.0545 37.3333 26.6667 34.9455 26.6667 32C26.6667 29.0545 29.0545 26.6667 32 26.6667C34.9455 26.6667 37.3333 29.0545 37.3333 32Z"
                                    stroke="#3C3C3C" stroke-width="2" />
                            </svg>
                            My Account
                        </button>
                        <button class="btn dash_tab_btn active">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="49" height="48" viewBox="0 0 49 48" fill="none">
                                <rect x="0.5" width="48" height="48" fill="url(#pattern0_5549_8230)" />
                                <defs>
                                    <pattern id="pattern0_5549_8230" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_5549_8230" transform="scale(0.00195312)" />
                                    </pattern>
                                    <image id="image0_5549_8230" width="512" height="512" preserveAspectRatio="none"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg>
                            Order History
                        </button>
                        <button class="btn dash_tab_btn" onclick="window.location.href='{{ route('logout') }}'">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="49" height="49" viewBox="0 0 49 49" fill="none">
                                <rect x="0.5" y="0.5" width="48" height="48" fill="url(#pattern0_5549_8317)" />
                                <defs>
                                    <pattern id="pattern0_5549_8317" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_5549_8317" transform="scale(0.00195312)" />
                                    </pattern>
                                    <image id="image0_5549_8317" width="512" height="512" preserveAspectRatio="none"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg>
                            Sign out
                        </button>
                    </div>
                </div>

                <div class="dash_tab_cutout_center">
                    <img src="{{ asset('frontend/BrandSparkz/assets/img/dash_tab_cutout_img.png') }}" alt="" class="img-fluid mobile_none dash_tab_cutout_img">
                    <img src="{{ asset('frontend/BrandSparkz/assets/img/dash_split.png') }}" alt="" class="img-fluid desktop_none dash_tab_cutout_img_mobo">
                </div>

                <div class="dash_tabs_movable_right">

                    <div class="order_right_inner">
                        <div class="order_top_div">
                            <h1 class="order_title mobile_none">Order ID: {{ $order->code }}</h1>
                            
                            
                            <button class="btn btn_global">
                                <img src="{{ asset('frontend/BrandSparkz/assets/img/btn_primary_pattern.png') }}" alt=""
                                    class="img-fluid btn_global_pattern">
                                <div class="btn_global_inner">
                                    <a href="{{ route('dashboard') }}" style="text-decoration:none;">
                                        <p class="cart_text">go back to order history</p>
                                    </a>
                                </div>
                            </button>
                            
                            
                            
                            
                            
                        </div>
                        <div class="order_center_div">
                            <div class="order_center_div_box">
                                <div class="order_center_div_box_top">
                                    <div class="orange_line"></div>
                                </div>
                                <div class="order_center_div_box_bottom">
                                    <div class="ocd_box">
                                        <img src="{{ asset('frontend/BrandSparkz/assets/img/orange_tic.svg') }}" alt="" class="img-fluid tic_1">
                                        <div class="orange_line_mob desktop_none"></div>
                                        <p class="order_center_div_box_text">Order Placed</p>
                                    </div>
                                    <div class="ocd_box">
                                        <img src="{{ asset('frontend/BrandSparkz/assets/img/orange_tic.svg') }}" alt="" class="img-fluid tic_2">
                                        <div class="orange_line_mob desktop_none"></div>
                                        <p class="order_center_div_box_text">On Review</p>
                                    </div>
                                    <div class="ocd_box">
                                        <img src="{{ asset('frontend/BrandSparkz/assets/img/orange_tic.svg') }}" alt="" class="img-fluid tic_3">
                                        <div class="orange_line_mob desktop_none"></div>
                                        <p class="order_center_div_box_text">Delivery Pending</p>
                                    </div>
                                    <div class="ocd_box">
                                        <img src="{{ asset('frontend/BrandSparkz/assets/img/orange_tic.svg') }}" alt="" class="img-fluid tic_4">
                                        <p class="order_center_div_box_text active">Delivered</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="order_content_div">
                            <div class="order_content_top">
                                <div class="order_content_box">
                                    <div class="order_content_box_top">
                                        <p class="order_content_title">Order Details</p>
                                    </div>
                                    <div class="order_content_box_bottom">
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Order No:</p>
                                            <p class="order_content_info_subtitle">{{ $order->code }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Date:</p>
                                            <p class="order_content_info_subtitle">{{ date('d-m-Y H:m A', $order->date) }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Status:</p>
                                            <p class="order_content_pill green">{{ ($order->payment_status == 'paid') ? 'Paid' : 'Failed' }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Total:</p>
                                            <p class="order_content_info_subtitle">{{ single_price($order->orderDetails->sum('price') + $order->orderDetails->sum('tax')) }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Payment method:</p>
                                            <p class="order_content_info_subtitle">Debit/Credit Card</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="order_content_box">
                                    <div class="order_content_box_top">
                                        <p class="order_content_title">Billing Details</p>
                                    </div>
                                    <div class="order_content_box_bottom">
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Name:</p>
                                            <p class="order_content_info_subtitle">{{ $order->shipping_address->name }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Email:</p>
                                                @if ($order->user_id != null)
                                                  <p class="order_content_info_subtitle">{{ $order->user->email }}</p>
                                                @endif
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Phone:</p>
                                            <p class="order_content_pill">{{ $order->shipping_address->phone }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Billing Address</p>
                                            <p class="order_content_info_subtitle">{{ $order->shipping_address->address }}, {{ $order->shipping_address->city }}, {{ $order->shipping_address->country }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="order_content_top">
                                <div class="order_content_box">
                                    <div class="order_content_box_top">
                                        <p class="order_content_title">Order Summary</p>
                                    </div>
                                    <div class="order_content_box_bottom">
                                        <table class="details-table table">
                                            <thead>
                                                <tr>
                                                    <th style="width: 0px;"></th>
                                                    <th><p class="order_t_title" style="margin-left: -48px;">Product</p></th>
                                                    <th><p class="order_t_title">Qty</p></th>
                                                    <th><p class="order_t_title">Price</p></th>
                                                    @if ($refund_request_addon != null && $refund_request_addon->activated == 1)
                                                        <th>{{__('Refund')}}</th>
                                                    @endif
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($order->orderDetails as $key => $orderDetail)
                                                <tr>
                                                    <td><p class="order_t_subtitle">{{ $key+1 }}</p></td>
                                                    <td>
                                                        @if ($orderDetail->product != null)
                                                            <a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank" class="order_t_sub_anchor">{{ $orderDetail->product->name }}</a>
                                                        @else
                                                            <strong>{{ __('Product Unavailable') }}</strong>
                                                        @endif
                                                    </td>
                                                    <td><p class="order_t_subtitle">{{ $orderDetail->quantity }}</p></td>
                                                    <td><p class="order_t_subtitle">{{ single_price($orderDetail->price) }}</p></td>
                                                    @if ($refund_request_addon != null && $refund_request_addon->activated == 1)
                                                        @php
                                                            $no_of_max_day = \App\Models\BusinessSetting::where('type', 'refund_request_time')->first()->value;
                                                            $last_refund_date = $orderDetail->created_at->addDays($no_of_max_day);
                                                            $today_date = Carbon\Carbon::now();
                                                        @endphp
                                                        <td>
                                                            @if ($orderDetail->product != null && $orderDetail->product->refundable != 0 && $orderDetail->refund_request == null && $today_date <= $last_refund_date && $orderDetail->delivery_status == 'delivered')
                                                                <a href="{{route('refund_request_send_page', $orderDetail->id)}}" class="order_t_sub_anchor">{{ __('Send') }}</a>
                                                            @elseif ($orderDetail->refund_request != null && $orderDetail->refund_request->refund_status == 0)
                                                                <span class="strong-600">{{ __('Pending') }}</span>
                                                            @elseif ($orderDetail->refund_request != null && $orderDetail->refund_request->refund_status == 1)
                                                                <span class="strong-600">{{ __('Approved') }}</span>
                                                            @elseif ($orderDetail->product->refundable != 0)
                                                                <span class="strong-600">{{ __('N/A') }}</span>
                                                            @else
                                                                <span class="strong-600">{{ __('Non-refundable') }}</span>
                                                            @endif
                                                        </td>
                                                    @endif
                                                </tr>
                                                 @endforeach
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <div class="order_content_box">
                                    <div class="order_content_box_top">
                                        <p class="order_content_title">Order Amount</p>
                                    </div>
                                    <div class="order_content_box_bottom">
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Subtotal</p>
                                            <p class="order_content_info_subtitle">{{ single_price($order->orderDetails->sum('price')) }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Tax</p>
                                            <p class="order_content_info_subtitle">{{ single_price($order->orderDetails->sum('tax')) }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Coupon Discount</p>
                                            <p class="order_content_info_subtitle">{{ single_price($order->coupon_discount) }}</p>
                                        </div>
                                        <div class="order_content_box_bottom_row">
                                            <p class="order_content_info_title">Total</p>
                                            <p class="order_content_info_subtitle">{{ currency_symbol().''.number_format($order->grand_total,2) }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>


            </div>

        </div>
    </section>
    
    
@endsection
@section('scripts')
<script>
      const logo = document.getElementById('header_logo');
      console.log(window.location.href)
      logo.src="{{ asset('frontend/BrandSparkz/assets/img/header_bg_dark_logo.svg') }}";
      
       const navbar = document.getElementById('header'); 
         navbar.classList.add('white_header'); 
        window.addEventListener('scroll', function () {
          const logochange = document.getElementById('header_logo');
             
        
          if (window.scrollY > 100) 
          {
            
            logochange.src="{{ asset('frontend/BrandSparkz/assets/img/header_bg_dark_logo.svg') }}";
             //navbar.classList.add('white_header'); 
           } else {
             
             logochange.src="{{ asset('frontend/BrandSparkz/assets/img/header_bg_trans_logo_.svg') }}";
              //navbar.classList.remove('white_header');  
          }
       });
</script>


@endsection